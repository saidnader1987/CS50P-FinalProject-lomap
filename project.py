...
if str(Path.cwd()) != r"/workspaces/90977733/cs50p/project":
    cwd = Path(r"/workspaces/90977733/cs50p/project")
else:
    cwd = Path.cwd()
...
class Bank:
    ...
    def bank(self, bank):
        ...
        if len(banks) != 0 and any(obj.bank == bank.lower().title() for obj in banks):
            raise ValueError("Invalid input: Bank already exists in the database.")
        ...
        while True:
            if new_bank := input("Bank: ").lower().title():
                ...
                if len(banks) != 0 and any(obj.bank == new_bank.lower().title() for obj in banks):
                    print("Invalid input: Bank already exists in the database. Banks in database:")
                    banks_report(banks)
                    continue
                else:
                    break
...
class Amortization:
    ...
    def value(self, value):
        try:
            ...
            if value > loan_principal_balance:
                raise ValueError(f"Amortization value (${value:,.2f}) can not be greater than loan balance (${loan_principal_balance:,.2f})")
            self._value = value
    ...
    def amort_date(self, amort_date):
        ...
        if not amort_date <= today:
            raise ValueError(f"Amortization date ({str(amort_date)}) can not be greater than today ({str(today)})")
        if not loan_issue_date < amort_date <= loan_maturity:
            raise ValueError(f"Amortization date ({str(amort_date)}) can not be less than or equal to loan issue date ({str(loan_issue_date)}) and can not be greater than loan maturity date ({str(loan_maturity)})")
    ...
    def edit(self):
        while True:
            # print("Please enter the following data. At any moment press CTRL + D to go back to the previous menu without saving.")
            option = input("Choose an option to edit: (v)alue, (a)mortization date: ").lower()
            # input for value
            if option == "v":
                ...
                while True:
                    if new_value :=  input("Amortization value: "):
                        try:
                            ...
                            print("Invalid input: Amortization value must be a positive number")
                            continue
                        else:
                            loan = get_obj(loans, self.loan_id, "id")
                            loan_principal_balance = loan.principal_balance
                    ...
                    if new_date :=  input("Amortization date: "):
                        try:
                            ...
                            if not new_date <= today:
                                print(f"Amortization date ({str(new_date)}) can not be greater than today ({str(today)})")
                                continue
                            elif not loan_issue_date < new_date <= loan_maturity:
                                print(f"Amortization date ({str(new_date)}) can not be less than or equal to loan issue date ({str(loan_issue_date)}) and can not be greater than loan maturity date ({str(loan_maturity)})")
                                continue
                            else:
                                break
                # Change amort in amortizations array
                ...
                print("Invalid input. Usage: v for editing amortization value, a for editing amortization date")
                continue
        ...
        if len(loans) != 0:
            ...
            while True:
                if loan_id := input("Loan id: "):
                    # if loan_id not in loans
                    if not any(str(obj.id) == str(loan_id) for obj in loans):
                        print(f"Invalid input: No loan with id: {loan_id} in database. Please enter one of the following loan ids:")
                        # for obj in loans:
            ...
            while True:
                if value := input("Amortization value: "):
                    try:
                        ...
                        loan_principal_balance = loan.principal_balance
                        if value > loan_principal_balance:
                            print(f"Amortization value (${value:,.2f}) can not be greater than loan balance (${loan_principal_balance:,.2f})")
                            continue
                        else:
                            break
            ...
            while True:
                if amort_date := input("Amortization date: "):
                    try:
                        ...
                        today = date.today()
                        if not amort_date <= today:
                            print(f"Amortization date ({str(amort_date)}) can not be greater than today ({str(today)})")
                            continue
                        elif not loan_issue_date < amort_date <= loan_maturity:
                            print(f"Amortization date ({str(amort_date)}) can not be less than or equal to loan issue date ({str(loan_issue_date)}) and can not be greater than loan maturity date ({str(loan_maturity)})")
                            continue
                        else:
                            break
...
class Loan:
    ...
    def bank(self, bank):
        ...
        if not any(obj.bank == bank.lower().title() for obj in banks):
            raise ValueError("Invalid input: Bank does not exist in the database.")
        # self._bank = bank.lower().title()
    ...
    def loan_term(self, loan_term):
        ...
        # if not type(loan_term) is int or loan_term <= 0:
            raise ValueError("Loan term must be a positive integer. Floating point numbers are not allowed.")
        else:
    ...
    def payment_frequency(self, payment_frequency):
        ...
        if not check_frequency(self.loan_term, payment_frequency):
            try:
                ...
                raise ValueError(f"Payment frequency and loan term does not match. Loan term ({self.loan_term}) not divisible by months in {payment_frequency} frequency ({period})")
    ...
    def interest_rate(self, interest_rate):
        ...
        # if type(interest_rate) not in [int, float] or interest_rate <= 0 or interest_rate > 100:
            raise ValueError("Interest rate must be a positive number between 0 and 100. Floating point numbers are allowed.")
        else:
    ...
    def interest_rate_type(self, interest_rate_type):
        if interest_rate_type.lower() not in TYPES:
            raise ValueError("Invalid input: Rate type does not exist in the database.")
    ...
    def nominal_rate_compounding_period(self, nominal_rate_compounding_period):
        if nominal_rate_compounding_period.lower() not in PERIODS:
            raise ValueError("Invalid input: Nominal rate compounding period does not exist in the database.")
            ...
            raise ValueError(f"Invalid input: Nominal rate compounding period can not be set to {nominal_rate_compounding_period} because interest rate type was set to nominal rate")
        else:
    ...
    def interest_payment_frequency(self, interest_payment_frequency):
        if not check_frequency(self.loan_term, interest_payment_frequency):
            try:
                ...
                raise ValueError(f"Interest payment frequency and loan term does not match. Loan term ({self.loan_term}) not divisible by months in {interest_payment_frequency} frequency ({period})")
                    ...
                    if new_face_value :=  input("Face value: "):
                        try:
                            ...
                            if new_face_value < self.face_value - self.principal_balance:
                                print(f"Face value (${new_face_value:,.2f}) can not be lesser than the sum of loan amortizations (${self.face_value - self.principal_balance:,.2f})")
                                continue
                            else:
                                break
                ...
                while True:
                    if new_bank := input("Bank: ").lower().title():
                        if not any(obj.bank == new_bank.lower().title() for obj in banks):
                            print("Invalid input: Bank does not exist in the database. Please enter one of the following banks:")
                            banks_report(banks)
                            continue
                        else:
                            break
            ...
            # input for issue date
                ...
                while True:
                    if new_issue_date := input("Issue date: "):
                        try:
                            new_issue_date = datetime.strptime(new_issue_date, '%Y-%m-%d').date()
                        except ValueError:
                            print("Invalid date format, should be YYYY-MM-DD")
                            continue
                        else:
                            ...
                            if len(self.actual_amortizations) != 0:
                                ...
                                min_amort_date = min(self.actual_amortizations, key = lambda x : x.amort_date).amort_date
                                max_amort_date = max(self.actual_amortizations, key = lambda x : x.amort_date).amort_date
                                new_maturity_date = new_issue_date + relativedelta(months = self.loan_term)
                                if new_issue_date >= min_amort_date:
                                    print(f"Issue date ({str(new_issue_date)}) can not be greater than or equal to min amortization date ({str(min_amort_date)})")
                                    continue
                                elif new_maturity_date < max_amort_date:
                                    print(f"Loan maturity ({str(new_maturity_date)}) can not be lesser than max amortization date ({str(max_amort_date)})")
                                    continue
                                else:
                                    break
                            else:
                ...
                while True:
                    if new_loan_term := input("Loan term: "):
                        try:
                            ...
                            print("Invalid input: Loan term must be a positive integer. Floating point numbers are not allowed.")
                            continue
                        else:
                            # checking if there are amortizations in loan
                            if len(self.actual_amortizations) != 0:
                                # checking actual amortization dates
                                max_amort_date = max(self.actual_amortizations, key = lambda x : x.amort_date).amort_date
                                new_maturity_date = self.issue_date + relativedelta(months = new_loan_term)
                                if new_maturity_date < max_amort_date:
                                    print(f"Loan issue date ({self.issue_date}) plus new loan term ({new_loan_term}) yields a new loan maturity date ({str(new_maturity_date)}) that is lesser than max amortization date ({str(max_amort_date)})")
                                    continue
                            ...
                            if not check_frequency(new_loan_term, self.payment_frequency):
                                ...
                                print(f"Payment frequency and loan term does not match. Loan term ({new_loan_term}) not divisible by months in {self.payment_frequency} frequency ({period})")
                            elif not check_frequency(new_loan_term, self.interest_payment_frequency):
                                period = MONTHS[self.interest_payment_frequency]
                                print(f"Interest payment frequency and loan term does not match. Loan term ({new_loan_term}) not divisible by months in {self.interest_payment_frequency} frequency ({period})")
                            else:
                                break
                # Change loan in loans array
                ...
                while True:
                    if new_payment_frequency := input("Payment frequency: ").lower():
                        if not new_payment_frequency in MONTHS:
                            print("Invalid input: Payment frequency does not exist in the database. Please enter one of the following payment frequencies:")
                            ...
                            print(f"Payment frequency and loan term does not match. Loan term ({self.loan_term}) not divisible by months in {new_payment_frequency} frequency ({period})")
                        else:
                            break
                # Change loan in loans array
                ...
                while True:
                    if new_interest_payment_frequency := input("Interest payment frequency: ").lower():
                        if not new_interest_payment_frequency in MONTHS:
                            print("Invalid input: Interest payment frequency does not exist in the database. Please enter one of the following payment frequencies:")
                            # for period in MONTHS.keys():
                            #     print(period)
                            # continue
                            print_frequencies()
                            ...
                            print(f"Interest payment frequency and loan term does not match. Loan term ({self.loan_term}) not divisible by months in {new_interest_payment_frequency} frequency ({period})")
                        else:
                            break
                # Change loan in loans array
                ...
                while True:
                    if new_interest_rate := input("Interest rate (number greater than 0 and less or equal than 100): "):
                        try:
                            ...
                            print("Invalid input: Interest rate must be a positive number less or equal than 100")
                            continue
                        else:
                            break
                # Change loan in loans array
                ...
                while True:
                    if new_interest_rate_type := input("Interest rate type: ").lower():
                        if new_interest_rate_type not in TYPES:
                            print("Invalid input: Rate type does not exist in the database. Please enter one of the following rate types:")
                            print_types()
                            continue
                        else:
                                ...
                                while True:
                                    if new_nominal_rate_compounding_period := input("Nominal rate compounding period: ").lower():
                                        if new_nominal_rate_compounding_period not in PERIODS:
                                            print("Invalid input: Nominal rate compounding period does not exist in the database. Please enter one of the following:")
                                            # for period in PERIODS.keys():
                                            #     print(period)
                                            print_periods()
                                            continue
                                        elif new_interest_rate_type == "nominal" and new_nominal_rate_compounding_period == "annually":
                                            print(f"Invalid input: Nominal rate compounding period can not be set to {new_nominal_rate_compounding_period} because interest rate type was set to nominal rate")
                                        else:
                                            break
                            break
                # Change loan in loans array
    ...
    def delete(self):
        ...
        if len(self.actual_amortizations) != 0:
            print("Loan can not be deleted because it has amortizations associated with it")
        else:
    ...
    def get(cls):
        if len(banks) != 0:
            ...
            while True:
                if payment_frequency := input("Payment frequency: ").lower():
                    if not payment_frequency in MONTHS:
                        ...
                        period = MONTHS[payment_frequency]
                        print(f"Payment frequency and loan term does not match. Loan term ({loan_term}) not divisible by months in {payment_frequency} frequency ({period})")
                    else:
                        break
                ...
                while True:
                    if nominal_rate_compounding_period := input("Nominal rate compounding period: ").lower():
                        if nominal_rate_compounding_period not in PERIODS:
                            ...
                            print(f"Invalid input: Nominal rate compounding period can not be set to {nominal_rate_compounding_period} because interest rate type was set to nominal rate")
                        else:
                            break
            ...
            while True:
                if interest_payment_frequency := input("Interest payment frequency: ").lower():
                    if not interest_payment_frequency in MONTHS:
                        ...
                        period = MONTHS[interest_payment_frequency]
                        print(f"Interest payment frequency and loan term does not match. Loan term ({loan_term}) not divisible by months in {interest_payment_frequency} frequency ({period})")
                    else:
                        break
            ...
            print("No banks in database. Please register a bank before registering a loan")
...
def menu(op):
    ...
    while True:
        if op == "main":
            ...
            option = input("Choose an option: (m)anage, (r)eports, (q)uit: ").lower()
            match option:
                case "m":
                    menu("manage")
                case "r":
            ...
            option = input("Choose an option: (l)oans, (a)mortizations, (b)anks, (g)o back, (q)uit: ").lower()
            match option:
                case "l":
                    menu("loans")
                case "a":
            ...
            option = input("Choose an option: (c)reate, (e)dit, (d)elete, (g)o back, (q)uit: ").lower()
            match option:
                case "c":
                    try:
                        ...
                        if loan := Loan.get():
                            ...
                            loan.update_sch()
                            loan.update_act()
                            append_csv_file(cwd / loans_path, LOAN_FIELDS, loan)
                    except EOFError:
                        print()
                            ...
                            try:
                                if loan_id := input("Please input the loan id you would like to edit (At any moment press CTRL + D to go back to the previous menu): "):
                                    ...
                                    if not any(str(obj.id) == str(loan_id) for obj in loans):
                                        ...
                                        loan = get_obj(loans, int(loan_id), "id")
                                        loans_report({loan})
                                        loan.edit()
                                        break
                            except EOFError:
                            ...
                            try:
                                if loan_id := input("Please input the loan id you would like to delete (At any moment press CTRL + D to go back to the previous menu): "):
                                    ...
                                    if not any(str(obj.id) == str(loan_id) for obj in loans):
                                        ...
                                        # get loan based on loan_id
                                        loans_report({get_obj(loans, int(loan_id), "id")})
                                        get_obj(loans, int(loan_id), "id").delete()
                                        break
                            except EOFError:
                                print()
                                break
                ...
                case "c":
                    try:
                        ...
                        if amortization := Amortization.get():
                            ...
                            amortizations.append(amortization)
                            # get loan based on amortization.loan_id and add amortization
                            loan = get_obj(loans, int(amortization.loan_id), "id")
                            loan.add_amortization(amortization)
                            loan.update_act()
                            append_csv_file(cwd / amort_path, AMORTIZATION_FIELDS, amortization)
                    except EOFError:
                        print()
                        print("Data not saved")
                case "e":
                            ...
                            try:
                                if amort_id := input("Please input the amortization id you would like to edit (At any moment press CTRL + D to go back to the previous menu): "):
                                    ...
                                    if not any(str(obj.id) == str(amort_id) for obj in amortizations):
                                        print(f"Invalid input: No amortization with id: {amort_id} in database. Please enter one of the following amortization ids:")
                                        amort_report(amortizations)
                                        continue
                                    else:
                                        # get amort based on amortization_id
                                        amortization = get_obj(amortizations, int(amort_id), "id")
                            ...
                            try:
                                if amort_id := input("Please input the amortization id you would like to delete (At any moment press CTRL + D to go back to the previous menu): "):
                                    ...
                                    if not any(str(obj.id) == str(amort_id) for obj in amortizations):
                                        ...
                                        # get amort based on amortization_id
                                        amort_report({get_obj(amortizations, int(amort_id), "id")})
                                        get_obj(amortizations, int(amort_id), "id").delete()
                                        break
                            except EOFError:
                                print()
                                break
                            ...
                            try:
                                if bank_id := input("Please input the bank id you would like to edit (At any moment press CTRL + D to go back to the previous menu): "):
                                    ...
                                    if not any(str(obj.id) == str(bank_id) for obj in banks):
                                        print(f"Invalid input: No bank with id: {bank_id} in database. Please enter one of the following bank ids:")
                                        banks_report(banks)
                                        continue
                                    else:
                                        # get bank based on bank_id
                                        bank = get_obj(banks, int(bank_id), "id")
                                        banks_report({bank})
                                        bank.edit()
                                        break
                            except EOFError:
                            ...
                            try:
                                if bank_id := input("Please input the bank id you would like to delete (At any moment press CTRL + D to go back to the previous menu): "):
                                    ...
                                    if not any(str(obj.id) == str(bank_id) for obj in banks):
                                        ...
                                        banks_report({get_obj(banks, int(bank_id), "id")})
                                        get_obj(banks, int(bank_id), "id").delete()
                                        break
                            except EOFError:
                                print()
                                break
            ...
            option = input("Choose an option: (l)oans, (a)mortizations, (b)anks, (c)ash flow, (g)o back, (q)uit: ").lower()
            match option:
                case "l":
                    loans_report(loans)
                case "a":
